...
function getCongesAnnuelValide($year,$mois,$employe_id=null,$jour_debut=null) {
...
// fonction pour la production : la table employes devient employers
function getCongesAnnuelValideProd($year,$mois,$employe_id=null,$jour_debut=null) {
...
try {
    ...
    DB::statement("create view v_conges_annuel_$year as select MONTH(c.debut),MONTH(c.fin),YEAR(c.debut),c.id, c.employe_id,type_conge_id,type_c.type_conge as type_conge,
                        ...
                        from conges c
                        JOIN employers emp on c.employe_id=emp.id
                        JOIN conges_heures_de_travail h_travail on emp.heure_de_travail_id=h_travail.id
                        JOIN conges_types_conge type_c on c.type_conge_id=type_c.id
                        where (c.etat_conge_id=1)
                        AND (YEAR(c.debut)=$year or YEAR(c.fin)=$year);"
...
function totalCongePayeEmploye($year,$mois,$employe_id) {
...
function totalCongeNonPayeEmploye($year,$mois,$employe_id) {
    $nombre_de_jour=getCongesAnnuelValide($year,$mois,$employe_id);
...
function getCongesAnnuelValide2($year,$mois=null,$employe_id=null,$jour_debut=null) {
    ...
    try {
        ...
        if ($mois==null) {
            $conges= DB::select('select * from v_conges_annuel_'.$year);
        } else {
    ...
    // return collect($jours_travail)->groupBy('employe_id');
